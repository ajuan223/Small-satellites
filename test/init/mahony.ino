...
void loop(){
    //读取传感器数据
    float g_x = Serial.print(gyro.gyro.x);
    float g_y = Serial.print(gyro.gyro.y);
    float g_z = Serial.print(gyro.gyro.z);

    float a_x = Serial.print(accel.acceleration.x);
    float a_y = Serial.print(accel.acceleration.y);
    float a_z = Serial.print(accel.acceleration.z);

    float m_x = Serial.print(event.magnetic.x);
    float m_y = Serial.print(event.magnetic.y);
    float m_z = Serial.print(event.magnetic.z);
    ...
    roll  = atan2(2.0f * (q0 * q1 + q2 * q3), q0q0 - q1q1 - q2q2 + q3q3);
    pitch = -asin(2.0f * (q1 * q3 - q0 * q2));
    yaw   = atan2(2.0f * (q1 * q2 + q0 * q3), q0q0 + q1q1 - q2q2 - q3q3);

    Serial.print("Roll: "); Serial.print(roll * 180.0f / M_PI);
    Serial.print("\t Pitch: "); Serial.print(pitch * 180.0f / M_PI);
    Serial.print("\t Yaw: "); Serial.println(yaw * 180.0f / M_PI);
    */
